...
**A merged or restructured section is a new section.** Re-derive what it is FOR and re-audit its opener against that; the old opener rarely survives the new job (Jul 2026 — two sections merged, and a mini-scale detail kept the power position while the merged section's real claim sat in sentence three. Zach: "Complete failure").
...
- **A name is a plain noun a person already knows** (Zach, Aug 7 2026). A document or feature NAME is a noun, full stop. A quantifier phrase is not a name ("Every Feature" — "is not A FUCKING NAME AT ALL"); trade vocabulary is not a name for a general reader ("Feature Inventory" — "No one knows what that is"); the surviving name was "Features." The noun census passes a bad name that is used consistently, so the name itself gets judged separately: would the reader say this word themselves?
...
- **Meta-theater** — the document performing its own process or virtue inside itself: "Completeness is the page's job," "still owed," "the checks make sure every link goes somewhere real," a rule quoted back with its own drama. Sales skeeze relocated into self-regard (Zach, Aug 7 2026: "I see you replacing skeaze with more skeaze"). Every sentence is about the subject; none is about the page, the process, or the writer's principles. Worst in standfirsts and footers, where contract-talk loves to live.
