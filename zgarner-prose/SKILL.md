...
- **unsayable** — a word or phrase that fails when spoken aloud: a mouthful ("adjudication" — "i can barelu say adjudication") or gross by connotation ("messy tail" — "evocative of shit"). Demo and workshop prose gets said to a room, so the test is saying the sentence out loud to a person. The fix is the word you'd actually use in a meeting: claims are processed or paid, unclear ones are flagged and sent to an examiner. "Escalation" is fine (Zach, Aug 7 2026). Coined Aug 7 2026.
- **jargon mashup** — a term of art from one field welded to another field's vocabulary into a compound that reads as neither: "the claims world" (RL's world-model idea wearing insurance words — Zach: "You're combining 'world model' and domain terminology. In a way that is inscrutable"). Kin to the jargon stack, but the failure is fusion, not piling: each half is fine alone and the compound means nothing to either audience. The same ruling covers a loaded infrastructure noun naming a plain artifact — "queue" for parquet files reads as Kafka ("does that mean you've set up like Kafka?"). The fix is the artifact's plain name: the claims data, 4,000 labeled claims in parquet. Coined Aug 8 2026.
