...
- **meta-theater** (also "self-referential contract prose") — a document performing its own process or virtue inside itself: "Completeness is the page's job," "a feature whose page is still owed," "the checks make sure every link goes somewhere real." The writer talking about the document instead of the document informing the reader — skeeze relocated from sales into self-regard. Zach, Aug 7 2026, on a standfirst full of it: "I see you replacing skeaze with more skeaze." The fix is sentences about the subject, none about the page.
- **a name is a plain noun** — a document's or feature's NAME must be a noun a person already knows. A quantifier phrase fails ("Every Feature" — "is not A FUCKING NAME AT ALL"); trade vocabulary fails ("Feature Inventory" — "No one knows what that is"); the answer was "Features." The noun census cannot catch a bad name: it checks consistency, and a bad name used consistently passes every check. Judge the name itself, separately, as register.
